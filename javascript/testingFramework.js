...
class ExpectFunctions {
    constructor(actual) {
      this.actual = actual;
      this.stringifieldActual = JSON.stringify(actual);
    }
  
    toExist() {
      if (this.actual == null) {
        throw `expected value to exist but got ${this.stringifieldActual}`;
      }
    }
  
    toBe(expected) {
      if (this.actual !== expected) {
        throw `expected ${this.stringifieldActual} to be ${JSON.stringify(
          expected,
        )}`;
      }
    }
